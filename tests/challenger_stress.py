...
DAEMON_BIN = os.environ.get(
    "DAEMON_BIN",
    "/home/roykevin/Projects/omarchy-sony/build/daemon/sony-headphones-daemon"
)
...
class DaemonContext:
    def __init__(self, sbox, name):
        ...
        self.sock_path = os.path.join(self.run_dir, "sony-headphones.sock")
        self.status_path = os.path.join(self.state_dir, "sony-headphones", "status.json")
        self.proc = None
...
# ---------------------------------------------------------------------------
def main():
    sbox = tempfile.mkdtemp(prefix="omasony_challenger_")
    print(f"============================================================")
