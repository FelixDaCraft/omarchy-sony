...
class DaemonHarness:
    def __init__(self):
        self.tmp_dir = tempfile.TemporaryDirectory(prefix="omasony_ipc_stress_")
        ...
        self.socket_path = self.runtime_dir / "sony-headphones.sock"
        self.daemon_proc = None
        bin_env = os.environ.get("DAEMON_BIN")
        if bin_env:
            self.daemon_bin = Path(bin_env)
        else:
            self.daemon_bin = Path(__file__).resolve().parent.parent / "build" / "daemon" / "sony-headphones-daemon"
...
def run_tests():
    ...
    def test_pipelined_commands():
        with harness.connect() as s:
            ...
            # Verify second status has updated values
            json2 = json.loads(lines[8])
            if json2.get("noise_mode") != "anc" or json2.get("ambient_level") != 12:
