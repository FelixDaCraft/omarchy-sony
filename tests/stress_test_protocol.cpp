...
static std::vector<std::vector<uint8_t>> generateSampleValidFrames() {
    ...
    frames.push_back(serializeAmbientLevel(20, false, 3));
    frames.push_back(serializeAmbientLevel(0, false, 4)); // Wind
    ...
    // Features
    frames.push_back(serializeSpeakToChat(true, 8));
    frames.push_back(serializeDsee(true, 9));
    frames.push_back(serializeMultipoint(true, 10));
    frames.push_back(serializeEarDetection(true, 11));

    // Queries
    frames.push_back(serializeQueryBattery(12));
    frames.push_back(serializeQueryNoiseMode(13));
    frames.push_back(serializeQueryEq(14));
    frames.push_back(serializeQueryDsee(15));
    frames.push_back(serializeQuerySpeakToChat(16));

    // Inbound-like responses
    std::vector<uint8_t> batRet = {0x23, 0x00, 75, 0x01};
    frames.push_back(packFrame(PacketType::DATA_MDR, 17, batRet));

    std::vector<uint8_t> ncRet = {0x67, 0x17, 0x01, 0x01, 0x01, 0x01, 15};
    frames.push_back(packFrame(PacketType::DATA_MDR, 18, ncRet));
...
void stressInboundPayloadSemantics() {
    ...
    std::vector<uint8_t> extremeBat = {0x23, 0x00, 255, 0x01};
    ...
    std::vector<uint8_t> extremeEq = {0x57, 0x00, 0xA0, 0x06, 0xFF, 0xFE, 0xFD, 0xFC, 0xFB, 0xFA};
...
int main() {
    std::cout << "=====================================================\n";
    std::cout << "  omarchy-sony: Adversarial Protocol Stress Harness  \n";
    std::cout << "=====================================================\n";
