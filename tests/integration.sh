...
# ------------------------------------------------------------------------------
CLI_BIN=""
if command -v sony-ctl >/dev/null 2>&1; then
  CLI_BIN="sony-ctl"
  CLI_TYPE="system binary ($(command -v sony-ctl))"
elif [[ -x "$REPO_ROOT/cli/build/sony-ctl" ]]; then
  CLI_BIN="$REPO_ROOT/cli/build/sony-ctl"
  CLI_TYPE="build binary ($CLI_BIN)"
elif [[ -x "$REPO_ROOT/build/cli/sony-ctl" ]]; then
  CLI_BIN="$REPO_ROOT/build/cli/sony-ctl"
  CLI_TYPE="build binary ($CLI_BIN)"
elif [[ -x "$HOME/.local/bin/sony-ctl" ]]; then
  CLI_BIN="$HOME/.local/bin/sony-ctl"
  CLI_TYPE="user bin ($CLI_BIN)"
else
  CLI_BIN="$SCRIPT_DIR/mock_sony_ctl.py"
  CLI_TYPE="built-in reference simulation client ($CLI_BIN)"
fi

echo -e "${BOLD}omarchy-sony End-to-End Simulation & Verification Test Suite${NC}"
...
# ------------------------------------------------------------------------------
# Feature 1.2: CLI (sony-ctl) Features (>=5 test cases)
# ------------------------------------------------------------------------------
...
if [[ $code -eq 0 ]] && echo "$out" | jq -e '.connected == true' >/dev/null 2>&1; then
  pass_test "T1.C1" "sony-ctl status exits 0 and outputs valid JSON"
else
  fail_test "T1.C1" "sony-ctl status returned code $code" "$out"
fi
...
set -e
if [[ $code -eq 0 ]]; then
  pass_test "T1.C2" "sony-ctl noise anc exits 0 on valid mode"
else
  fail_test "T1.C2" "sony-ctl noise anc returned code $code"
fi
...
set -e
if [[ $code -eq 0 ]]; then
  pass_test "T1.C3" "sony-ctl ambient-level 12 exits 0 on valid level"
else
  fail_test "T1.C3" "sony-ctl ambient-level 12 returned code $code"
fi
...
if [[ $code1 -eq 0 && $code2 -eq 0 ]]; then
  pass_test "T1.C4" "sony-ctl eq commands exit 0 on valid presets and custom bands"
else
  fail_test "T1.C4" "sony-ctl eq commands failed (code1=$code1, code2=$code2)"
fi
...
if [[ $code1 -eq 1 && $code2 -eq 1 && $code3 -eq 1 ]]; then
  pass_test "T1.C6" "sony-ctl exits with code 1 on invalid arguments and unknown commands"
else
...
set -e
if [[ $code -eq 2 ]]; then
  pass_test "T1.C7" "sony-ctl exits with code 2 when daemon socket is offline"
else
...
# T2.B7: Ambient sound level boundary values (0 and 20 exact, <0 and >20 boundary)
set +e
...
c_under=$?
run_cli ambient-level 21 >/dev/null 2>&1
c_over=$?
run_cli ambient-level not_a_number >/dev/null 2>&1
c_nan=$?
set -e
if [[ $c_zero -eq 0 && $c_twenty -eq 0 && $c_under -eq 1 && $c_over -eq 1 && $c_nan -eq 1 ]]; then
  pass_test "T2.B7" "Ambient level boundary values [0, 20] accepted; out-of-range rejected with code 1"
else
  fail_test "T2.B7" "Ambient level boundary check failed" "c0=$c_zero, c20=$c_twenty, c_under=$c_under, c_over=$c_over, c_nan=$c_nan"
fi
...
if jq -e '.battery_level == 150 and .ambient_sound_level == 99' "$STATUS_FILE" >/dev/null 2>&1; then
...
if [[ "$b1" -eq 85 && "$b2" -eq 15 && "$b3" == "92:true" ]]; then
  pass_test "T3.S3" "Battery warning and charging state transitions (85% -> 15% alert -> 92% charging)"
else
...
if [[ "$final_level" -eq 20 ]]; then
  pass_test "T3.S5" "Sequential command stream executes and resolves to final value (level 20)"
else
  fail_test "T3.S5" "Rapid sequential commands did not settle to 20 (got $final_level)"
fi
